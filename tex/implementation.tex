...
\end{equation}
The amount of PCs, $n$, was chosen as a minimum number of dimensions required to preserve 95\% of the data's total variance so that that the sum of the explained variances of  $n$ PCs was equal to at least 0.95 (usually between 4---6 PCs).
...
\footnotesize
    ...
    \Input{number of clusters $k$, \\matrix describing the features of the samples $A$ = (n\_samples, n\_features)}
    \Output{entities divided into clusters}
    \BlankLine
    
    ramdomly select $k$ clusters\\
    
	\Repeat{clusters centroids do not change}{
